...
const InjuryModal = ({ isOpen, onClose, teamName, sport, injuryData, loading }) => {
  ...
  const renderInjuryCard = (injury, index) => (
    <div key={index} className="model_content" style={{
      ...
      <div style={{ color: '#fff', fontSize: '14px', lineHeight: '1.4' }}>
        <strong>Description:</strong> {injury.description || 'No description available'}
      </div>
    </div>
  );
  ...
  return (
    <div className="modal-overlay" style={{
      ...
      <div className="modal-content injury_model" style={{
            ...
            <div>
              {homeTeamData.length > 0 && (
                <div style={{ marginBottom: '30px' }}>
                  <h4 style={{ marginBottom: '15px', color: '#333', borderLeft: '4px solid #007bff', paddingLeft: '10px' }}>
                    {homeTeamData[0]?.teams_name || 'Home Team'} Injuries:
                  </h4>
                  {homeTeamData.map((injury, index) => renderInjuryCard(injury, `home-${index}`))}
                </div>
              )}
              
              {awayTeamData.length > 0 && (
                <div>
                  <h4 style={{ marginBottom: '15px', color: '#333', borderLeft: '4px solid #28a745', paddingLeft: '10px' }}>
                    {awayTeamData[0]?.teams_name || 'Away Team'} Injuries:
                  </h4>
                  {awayTeamData.map((injury, index) => renderInjuryCard(injury, `away-${index}`))}
                </div>
              )}
            </div>
          ) : (
